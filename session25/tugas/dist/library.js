...
export class LibraryService {
    ...
    searchBooks(query) {
        ...
        return [...db.books.values()].filter((b) => b.title.toLowerCase().includes(q) ||
    ...
    checkoutBook(params) {
        ...
        if (!member.isActive)
            throw new PermissionError("Member not active");
        if (book.availableCopies <= 0)
            throw new ConflictError("No available copies");
        const activeLoansForMember = [...db.loans.values()].filter((l) => l.memberId === member.id && l.status !== "RETURNED");
    ...
    listLoans(filter) {
        ...
        for (const l of items) {
            if (l.status === "ONGOING" && l.dueAt < nowDate)
                l.status = "LATE";
        }
        return items;
    }
